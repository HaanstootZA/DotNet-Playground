...
namespace MappingApplication
{
    class MapGenerator
    {
        ...
        private double GetMinRad(SizeF Size) { return Math.Sqrt(Math.Pow(Size.Width, 2) + Math.Pow(Size.Height, 2)) / 2; }
        ...
        private void SetCoordinates(Layer RootLayer)
        {
            ...
            SizeF cSize = new SizeF(0f, 0f);
            double rad = 0;
            while (currentLayer != null)
            {
                cSize = currentLayer.MaxSize;
                MapMath.MINRAD += GetMinRad(maxsize);
                MapMath.MINRAD += GetMinRad(cSize);
                if (max == 0)
